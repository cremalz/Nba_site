...
document.getElementById('ghost-number').textContent =
  jerseys[Math.floor(Math.random() * jerseys.length)];
...
const TROPHY_SVG_SMALL = TROPHY_SVG.replace(
  'class="trophy-svg glow"',
  'class="trophy-svg glow" style="width:18px;height:24px;margin-right:2px"'
);
...
async function loadGames() {
  ...
  try {
    ...
    if (!events.length) {
      list.innerHTML = '<p class="empty-msg">Sem jogos agendados para hoje.</p>';
      return;
    }
    ...
    list.innerHTML = events.map(ev => {
      ...
      const seriesText = comp.series
        ? `Série: ${comp.series.completed ? 'Encerrada' : comp.series.title || ''}`
        : '';
      ...
      const venue = comp.venue
        ? `<span class="game-venue">${comp.venue.fullName?.split(',')[0] || ''}</span>`
        : '';
    ...
    list.innerHTML = '<p class="empty-msg">Não foi possível carregar os jogos.</p>';
  }
}
...
async function loadStandings() {
  ...
  try {
    ...
    if (!confs.length) throw new Error('no data');
    ...
    wrap.innerHTML = '<p class="empty-msg" style="grid-column:1/-1">Não foi possível carregar a classificação.</p>';
  }
}
